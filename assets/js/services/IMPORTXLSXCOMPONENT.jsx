...
import { API_COMPONENNT, API_PRODUCT } from './Config';

const IMPORTXLSXCOMPONENT = () => {
        ...
        if (items.length > 0) {
            ...
            for (let i = 0; i < items.length; i++) {
                const saveData = async (data) => {
                    try {
                        const response = await axios.post(API_COMPONENNT, items[i])
...
export default IMPORTXLSXCOMPONENT